...
export const PACKAGE_UNITS: UnitDef[] = [
...
export const ALL_UNITS: UnitDef[] = [...BASE_UNITS, ...PACKAGE_UNITS]

const PACKAGE_IDS = new Set(PACKAGE_UNITS.map((u) => u.id))
export const isPackageUnit = (unitId: string): boolean => PACKAGE_IDS.has(unitId)

/** Caja / paquete / panal: docena se modela como paquete con N unidades. */
const ANCHOR_STOCK_PACKAGE_IDS = new Set(['box', 'pack', 'panal'])
export const showsAnchorStockLink = (displayUnitId: string): boolean =>
  ANCHOR_STOCK_PACKAGE_IDS.has(displayUnitId)

export const ALL_UNITS_MAP: Record<string, UnitDef> = Object.fromEntries(
  ALL_UNITS.map((u) => [u.id, u]),
)

export function formatProductUnit(product: Product): string {
 ...
 */
export function packageContentRowLabel(displayUnitId: string): string {
