...
function PendingRegistrationPanel({ householdId, products }) {
  ...
  const [pdfImportStatus, setPdfImportStatus] = useState<{ type: string; message: string; progress?: number }>({ type: 'idle', message: '' })
  ...
  const handleBatchPdfChange = async (e) => {
    ...
    try {
      ...
      setPdfImportStatus({
        type: 'error',
        message: (err as Error)?.message ? String((err as Error).message) : 'No se pudo leer el PDF.',
      })
    }
  }
  ...
  const handleBatchXmlChange = async (e) => {
    ...
    try {
      ...
      setPdfImportStatus({
        type: 'error',
        message: (err as Error)?.message ? String((err as Error).message) : 'No se pudo leer el XML.',
      })
    }
  }
    ...
    try {
      ...
      if (invErr || !inv) {
        setAlertDialog({
          ...
          title: 'Factura',
          message:
            (invErr as Error)?.message || 'No se pudo crear el registro de factura. Inténtalo de nuevo.',
        })
        return
      }
      ...
      try {
        ...
        setAlertDialog({
          ...
          message: (recErr as Error)?.message
            ? `Error al guardar líneas: ${(recErr as Error)?.message}`
  ...
  const handleBatchSubmit = async (e: React.FormEvent) => {
    ...
    interface SubmitLine { productId: string; qty: number; unitPrice: number; lineTotal: number }
    const validLines: SubmitLine[] = []
    for (const line of batchForm.lines) {
      if (!line.productId) continue
      const qty = parseFloat(String(line.quantity).replace(',', '.'))
      const unitPrice = paidToUnitPrice(line.price, line.quantity)
