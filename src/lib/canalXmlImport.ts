...
function titleCaseDesc(s: unknown): string {
  const t = String(s || '')
    .trim()
    .replace(/\s+/g, ' ')
...
export function normKey(desc: unknown): string {
  return String(desc)
    .replace(/\*/g, ' ')
    .toUpperCase()
    .normalize('NFD')
...
export function buildNormNameToProductIdMap(products: Product[]): Map<string, UUID> {
 ...
 */
function ublLinePayableTotalCop(lineSegment: string): number | null {
...
function parseUblTaxInclusiveCop(innerInvoiceXml: string): number | null {
...
function allocateLineTotalsCop(rawTotals: number[], target: number | null): number[] {
 ...
 */
export function parseUblInvoiceLineItems(innerInvoiceXml: string): ParsedInvoiceItem[] {
...
export function extractEmbeddedInvoiceXml(fileText: string): string | null {
...
function parseUblIssueDate(inner: string): string | null {
  const m = inner.match(/<cbc:IssueDate>([^<]+)<\/cbc:IssueDate>/)
  return m ? m[1].trim() : null
}

function parseUblPayableAmountCop(inner: string): number | null {
 ...
 */
export function parseElectronicInvoiceXmlForBatch(xmlText: string): ParsedInvoicePayload {
 ...
 */
export function applyNameFallbackToBatchLines(lines: BatchLine[], products: Product[]): BatchLine[] {
  const nameMap = buildNormNameToProductIdMap(products)
  return lines.map((line) => {
    if (line.productId) return line
    const desc = line.invoiceDesc || ''
    const nk = normKey(desc)
    const pid = nk && nameMap.has(nk) ? (nameMap.get(nk) as UUID) : ''
