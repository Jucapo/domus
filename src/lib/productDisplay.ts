...
export type ProductMetaChipKey = 'brand' | 'barcode' | 'content'

export interface ProductMetaChip {
  key: ProductMetaChipKey
  label: string
...
export function productMetaChipClassName(key: ProductMetaChipKey | string): string {
  switch (key) {
    case 'brand':
...
export function buildGastosProductMetaChips(product: Product): ProductMetaChip[] {
...
export function buildProductMetaChips(product: Product): ProductMetaChip[] {
  const chips: ProductMetaChip[] = []
  if (product.brand) {
    const b = toTitleCase(product.brand)
    chips.push({ key: 'brand', label: b, desktopPrefix: 'Marca:', desktopValue: b })
...
export function productUnitSummaryLine(product: Product): string {
